...
def generate_many_stats(data_dict: dict,
    ...
    Args:
        ...
        to_np: enforce converting torch.Tensor to np.ndarray in the result
        exception_keys: keys to be excluded in the statistics
...
def grad_norm_clip(bound, params):
    ...
    grads = [p.grad.detach().view(-1) for p in params if
             p.requires_grad and p.grad is not None]

    if grads:
        # Compute gradient norm before clipping
        grad_norm_before = torch.norm(torch.cat(grads), p=2).item()

        # Clipping
        if bound > 0:
            grad_norm_clipped \
                = torch.nn.utils.clip_grad_norm_(params, bound).item()

        else:
            grad_norm_clipped = grad_norm_before
    else:
...
def grad_from_each_loss(nn, optimizer, *losses):
    """
    Compute the gradient of each loss and return the gradients
    Args:
        nn: NN model
    ...
    Returns:
        gradients of each loss
    """
    # Compute the gradients of each loss
    grad_norm_list = []
    for loss in losses:
        if loss == 0:
            ...
            grads = [p.grad.detach().view(-1) for p in nn.parameters if
                     p.requires_grad and p.grad is not None]
            grad_norm = torch.norm(torch.cat(grads), p=2).item()
