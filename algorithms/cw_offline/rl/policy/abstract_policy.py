...
import torch
from algorithms.cw_offline.util import MLP, TrainableVariable
...
class AbstractGaussianPolicy(ABC):
    ...
    def copy_(self, policy, rate=1):
        assert 0 < rate <= 1, "Rate should be in [0, 1]"
        for self_param, policy_param in zip(self.parameters, policy.parameters):
            self_param.data = ((1 - rate) * self_param.data
                               + rate * policy_param.data)

    def copy_parameter(self, parameters):
        for self_param, param in zip(self.parameters, parameters):
            self_param.data = param.data
