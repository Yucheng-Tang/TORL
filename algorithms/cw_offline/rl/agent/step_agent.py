...
from algorithms.cw_offline.rl.critic import CQLCritic, IQLCritic
...
class StepQAgent(AbstractAgent):
    def __init__(self,
        ...
        self.is_offline = self.sampler is None
        self.use_projection = self.projection is not None
        ...
        self.log_now = False
        self.random_target = kwargs.get("random_target", True)  # Double Q-learning
        self.num_samples_in_targets = kwargs.get("num_samples_in_targets", 1)  # target Q-values
        ...
        # For off-policy/offline learning
        self.replay_buffer = replay_buffer
        self.return_type = kwargs.get("return_type")
        self.batch_size = kwargs.get("batch_size")

        # Optinal params for Offline
    ...
    def step(self):
        ...
        # Collect dataset (Online)
        if not self.is_offline:  # TODO:not modified for stepAgent
        ...
        # Done by load_offline_dataset(dataset) in main function
    ...
    @torch.no_grad()
    def load_offline_dataset(self, dataset: dict):
        """
        Loads an offline dataset (e.g., from D4RL) into the replay buffer.
        Expects keys like: observations, actions, rewards, next_observations, terminals
        """
        if self.replay_buffer._size != 0:
            raise ValueError("Replay buffer is not empty.")

        dataset_dict = {
            "observations": torch.tensor(dataset["observations"], dtype=torch.float32, device=self.device),
            "actions": torch.tensor(dataset["actions"], dtype=torch.float32, device=self.device),
            "rewards": torch.tensor(dataset["rewards"][..., None], dtype=torch.float32, device=self.device),
            "next_observations": torch.tensor(dataset["next_observations"], dtype=torch.float32, device=self.device),
            "dones": torch.tensor(dataset["terminals"][..., None], dtype=torch.bool, device=self.device)
        }

        self.replay_buffer.add(dataset_dict)
        print(f"[StepQAgent] Loaded offline dataset with {len(dataset['observations'])} transitions.")
