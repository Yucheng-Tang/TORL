...
from algorithms.cw_offline.rl.critic.abstract_critic import AbstractCritic
from algorithms.cw_offline.rl.policy.abstract_policy import AbstractGaussianPolicy
from algorithms.cw_offline.rl.sampler.abstract_sampler import AbstractSampler
...
class AbstractAgent(ABC):
    ...
    def get_lr_scheduler(self, epoch=-1):
        """
        ...
        if self.schedule_lr_policy:
            policy_lr_scheduler = \
                LinearLR(self.policy_optimizer,
                         start_factor=1,
                         end_factor=0.01,
                         total_iters=self.total_iterations, last_epoch=epoch)
        else:
            policy_lr_scheduler = None
        if self.schedule_lr_critic:
            # we may have 1 or more critic optimizers
            list_critic_lr_scheduler = []
            critic_optimizers = util.make_iterable(self.critic_optimizer)
            for critic_opt in critic_optimizers:
                critic_lr_scheduler = \
                    LinearLR(critic_opt,
                             start_factor=1,
                             end_factor=0.01,
                             ...
                             last_epoch=epoch)
                list_critic_lr_scheduler.append(critic_lr_scheduler)
        else:
            list_critic_lr_scheduler = None
        return policy_lr_scheduler, list_critic_lr_scheduler
            ...
            for i, critic_opt in enumerate(self.critic_optimizer):
                critic_optimizer_path = \
                    util.get_training_state_save_path(log_dir,
                                                      ...
                                                      epoch)
                with open(critic_optimizer_path, "wb") as critic_opt_file:
                    torch.save(critic_opt.state_dict(), critic_opt_file)
