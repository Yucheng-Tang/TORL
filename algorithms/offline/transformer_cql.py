...
class ContinuousCQL:
    ...
    def _compute_segment_n_step_returns(
            self,
            observations: torch.Tensor,
            actions: torch.Tensor,  # Add dataset actions parameter
            rewards: torch.Tensor,
            dones: torch.Tensor,
            idx_in_segments: torch.Tensor,
            seg_start_idx: torch.Tensor,
    ) -> torch.Tensor:
        """
        Segment-wise n-step return computation 
        
        Args:
            observations: [num_traj, traj_length, dim_state]
            rewards: [num_traj, traj_length]
            dones: [num_traj, traj_length]
            idx_in_segments: [num_segments, segment_length + 1]
            seg_start_idx: [num_segments]
            
        Returns:
            n_step_returns: [num_traj, num_segments, 1 + num_seg_actions]
        """
        num_traj, traj_length = observations.shape[:2]
        num_segments = idx_in_segments.shape[0]
        num_seg_actions = idx_in_segments.shape[-1] - 1
        
        # Get segment start indices
        seg_start_idx = idx_in_segments[..., 0]  # [num_segments]
        
        with torch.no_grad():
            # Get action dimension from the actor or use a default
            action_dim = getattr(self.actor, 'action_dim', None)
            if action_dim is None:
                # Try to get from actor parameters or use a reasonable default
                action_dim = getattr(self.actor, 'action_dim', 6)  # Default for most environments
            
            # Generate policy actions for each segment (like original seq_agent.py)
            # The actions are generated by the current policy, not from dataset
            actions_for_critic = torch.zeros([num_traj, traj_length, num_seg_actions, action_dim], 
                                           device=self._device)
            
            # Sample actions for each segment using the policy (like original seq_agent.py)
            for i in range(num_segments):
                start_idx = seg_start_idx[i]
                end_idx = min(start_idx + num_seg_actions, traj_length)
                
                if start_idx < traj_length:
                    for t in range(start_idx, end_idx):
                        # Get observation for this specific timestep
                        timestep_obs = observations[:, t:t+1]  # [num_traj, 1, dim_state]
                        
                        # Sample one action for this timestep using current policy
                        timestep_actions, _ = self.actor.sample(timestep_obs)  # [num_traj, 1, dim_action]
                        
                        # Store the policy-generated action for this timestep
                        segment_pos = t - start_idx
                        if segment_pos < num_seg_actions:
                            actions_for_critic[:, t, segment_pos] = timestep_actions[:, 0]  # Remove the singleton dimension
            
            # Use ALL states along the trajectory (like original seq_agent.py)
            c_state = observations  # [num_traj, traj_length, dim_state]
            
            # Create indices for ALL timesteps (like original seq_agent.py)
            c_idx = torch.arange(traj_length, device=self._device).long()
            
            # Handle num_samples_in_targets (like original seq_agent.py)
            num_samples_in_targets = getattr(self, 'num_samples_in_targets', 1)
            if num_samples_in_targets == 1:
                # [num_times] -> [num_traj, traj_length]
                c_idx = util.add_expand_dim(c_idx, [0], [num_traj])
            else:
                num_smp = num_samples_in_targets
                # [num_times] -> [num_traj, traj_length, num_smp]
                c_idx = util.add_expand_dim(c_idx, [0, 2], [num_traj, num_smp])
            
            # Create indices for ALL actions (like original seq_agent.py)
            a_idx = util.add_expand_dim(c_idx, [-1], [num_seg_actions])
            a_idx_evo = torch.arange(num_seg_actions, device=self._device).long()
            a_idx = a_idx + a_idx_evo
            
            # Use mix precision for faster computation (like original seq_agent.py)
            with util.autocast_if(self.use_mix_precision):
                # Compute Q-values using the critic's critic method (like original seq_agent.py)
                future_q1 = self.critic.critic(
                    self.critic.target_net1, 
                    d_state=None, 
                    c_state=c_state,  # ALL states
                    actions=actions_for_critic,   # Policy-generated actions (like original seq_agent.py)
                    idx_d=None, 
                    idx_c=c_idx,      # ALL timestep indices
                    idx_a=a_idx       # ALL action indices
                )
                
                # Handle single_q check (like original seq_agent.py)
                if not getattr(self.critic, 'single_q', False):
                    future_q2 = self.critic.critic(
                        self.critic.target_net2, 
                        d_state=None, 
                        c_state=c_state,  # ALL states
                        actions=actions_for_critic,   # Policy-generated actions (like original seq_agent.py)
                        idx_d=None, 
            ...
            # Handle random_target logic (like original seq_agent.py)
            random_target = getattr(self, 'random_target', False)
            if random_target:
                # Randomly choose the Q value from Q1 or Q2
                mask = torch.randint(0, 2, future_q1.shape, device=self._device)
            ...
            # Handle num_samples_in_targets > 1 averaging (like original seq_agent.py)
            if num_samples_in_targets > 1:
                min_q1_q2 = min_q1_q2.mean(dim=-2)
            
            # Note: Handle Q-func beyond the length of the trajectory (like original seq_agent.py)
            future_q = torch.zeros([num_traj, traj_length], device=self._device)
            
            # Use the last q in the sequence if time is within the traj length
            future_q[:, :-num_seg_actions] = min_q1_q2[:, :-num_seg_actions, -1]
            
            # Use the q of the last traj time if actions is beyond the traj length
            for i in range(num_seg_actions):
                future_q[:, -num_seg_actions + i] = min_q1_q2[:, -num_seg_actions + i, -1 - i]
            
            ################ Compute the reward in the future ##################
            # [num_traj, traj_length] -> [num_traj, traj_length + num_seg_actions]
            future_r_pad_zero_end = torch.nn.functional.pad(rewards, (0, num_seg_actions))
            
            # discount_seq as: [1, gamma, gamma^2..., 0] (like original seq_agent.py)
            discount_idx = torch.arange(traj_length + num_seg_actions, device=self._device)
            discount_seq = torch.tensor(self.discount, device=self._device).pow(discount_idx)
            discount_seq[-num_seg_actions:] = 0
            
            # Apply discount to all rewards and returns w.r.t the traj start (like original seq_agent.py)
            # [num_traj, traj_length + 1]
            future_q_pad_zero_end = torch.nn.functional.pad(future_q, (0, num_seg_actions))
            discount_q = future_q_pad_zero_end * discount_seq
            
            # [num_traj, traj_length + 1]
            discount_r = future_r_pad_zero_end * discount_seq
            
            # -> [num_traj, num_segments, traj_length + 1] (like original seq_agent.py)
            discount_r = util.add_expand_dim(discount_r, [1], [num_segments])
            
            # [num_traj, num_segments, num_seg_actions + 1]
            seg_discount_q = discount_q[:, idx_in_segments]
            
            # -> [num_traj, num_segments, num_seg_actions + 1] (like original seq_agent.py)
            seg_reward_idx = util.add_expand_dim(idx_in_segments, [0], [num_traj])
            
            # torch.gather shapes (like original seq_agent.py)
            # input: [num_traj, num_segments, traj_length + 1]
            # index: [num_traj, num_segments, num_seg_actions + 1]
            # result: [num_traj, num_segments, num_seg_actions + 1]
            seg_discount_r = torch.gather(input=discount_r, dim=-1, index=seg_reward_idx)
            
            # [num_traj, num_segments, num_seg_actions + 1] ->
            # [num_traj, num_segments, num_seg_actions + 1, num_seg_actions + 1] (like original seq_agent.py)
            seg_discount_r = util.add_expand_dim(seg_discount_r, [-2], [num_seg_actions + 1])
            
            # Get a lower triangular mask with off-diagonal elements as 1 (like original seq_agent.py)
            # [num_seg_actions + 1, num_seg_actions + 1]
            reward_tril_mask = torch.tril(torch.ones(num_seg_actions + 1, num_seg_actions + 1, device=self._device), diagonal=-1)
            
            # [num_traj, num_segments, num_seg_actions + 1, num_seg_actions + 1]
            tril_discount_rewards = seg_discount_r * reward_tril_mask
            
            # Compute discount start for each segment (like original seq_agent.py)
            discount_start = torch.tensor(self.discount, device=self._device).pow(seg_start_idx)[None, :, None]
            
            # Compute n-step returns
            # V(s0) -> R0
            # Q(s0, a0) -> r0 + γ * R1
            # Q(s0, a0, a1) -> r0 + γ * r1 + γ² * R2
            # Q(s0, a0, a1, a2) -> r0 + γ * r1 + γ² * r2 + γ³ * R3
            
            # [num_traj, num_segments, 1 + num_seg_actions]
            n_step_returns = (tril_discount_rewards.sum(dim=-1) + seg_discount_q) / discount_start
    ...
    def train(self, batch: TensorBatch) -> Dict[str, float]:
        ...
        #                             Update critic                            #
        ########################################################################
        util.run_time_test(lock=True, key="update critic")        # Check if critic is SeqCritic
        ...
        if isinstance(self.critic, seq_critic.SeqCritic):
            ...
            if self.return_type == "segment_n_step_return_vf":
                ...
                # qf_loss, alpha_prime, alpha_prime_loss = self._segment_n_step_return_vf(
                #     observations_seq,
                #     actions_seq,
                #     rewards_seq,
                #     dones_seq,
...
def train(config: TrainConfig, cw_config: dict = None) -> None:
    ...
    # target_critic_1 = deepcopy(critic_1).to(config.device)
    # target_critic_2 = deepcopy(critic_2).to(config.device)
