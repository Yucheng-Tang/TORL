...
class ContinuousCQL:
    def __init__(
        ...
        self.actor_optimizer_t = actor_optimizer_t
        if self.use_automatic_entropy_tuning:
            self.log_alpha_t = Scalar(0.0)
            self.alpha_optimizer_t = torch.optim.Adam(
                self.log_alpha_t.parameters(),
                lr=self.policy_lr,
            )
        else:
            self.log_alpha_t = None

        self.critic = critic_t
        ...
        self.progress_bar = tqdm(total=max_timesteps)
        self.log_now = False
    ...
    def train(self, batch: TensorBatch) -> Dict[str, float]:
        ...
        log_dict = dict(
            log_pi_original=log_pi.mean().item(),
            policy_loss_original=policy_loss.item(),
            alpha_loss_original=alpha_loss.item(),
            alpha_original=alpha.item(),
        )

        """ Q function loss """
        qf_loss, alpha_prime, alpha_prime_loss = self._q_loss(
            observations_step, actions_step, next_observations_step, rewards_step, dones_step, alpha, log_dict
        )
        ...
        #                             Update Policy                            #
        ########################################################################
        new_actions_t, log_pi_t = self.actor_t(observations_step)
        if self.num_samples_in_policy == 1:
            new_actions_t = new_actions_t.squeeze()
        alpha_t, alpha_loss_t = self._alpha_and_alpha_loss(observations_step, log_pi_t)
        policy_loss_t = self._policy_loss_t(
            observations_step, actions_step, new_actions_t, alpha_t, log_pi_t
        )

        log_dict.update(
            dict(
                log_pi_t=log_pi_t.mean().item(),
                policy_loss_t=policy_loss_t.item(),
                alpha_loss_t=alpha_loss_t.item(),
        ...
        if self.return_type == "segment_n_step_return_vf":
            ...
            for net_name, net, target_net, opt, scaler in self.critic_nets_and_opt():
                ...
                with (util.autocast_if(self.use_mix_precision)):
                    ...
                    vq_pred_masked = vq_pred.clone()
                    targets_masked = targets.clone()
                    vq_pred_masked[..., 1:] = vq_pred[..., 1:] * valid_mask
                    targets_masked[..., 1:] = targets[..., 1:] * valid_mask

                    vq_diff = vq_pred_masked[..., 1:] - targets_masked[..., 1:]
                    critic_loss = mse(vq_pred_masked[..., 1:], targets_masked[..., 1:])
            ...
            if self.use_automatic_entropy_tuning:
                self.alpha_optimizer_t.zero_grad()
                alpha_loss_t.backward()
                self.alpha_optimizer_t.step()

            self.actor_optimizer_t.zero_grad()
            policy_loss_t.backward()
            self.actor_optimizer_t.step()

            # Update critic after policy
...
def train(config: TrainConfig):
    ...
    critic_t = seq_critic.SeqCritic(**critic_config)

    actor_t = TanhGaussianPolicy(
        state_dim,
        action_dim,
        max_action,
        log_std_multiplier=config.policy_log_std_multiplier,
    ...
    actor_optimizer_t = torch.optim.Adam(actor_t.parameters(), config.policy_lr)
    ...
    kwargs = {
        ...
        # TCQL
        "actor_t": actor_t,
        "actor_optimizer_t": actor_optimizer_t,
        "critic_t": critic_t,
    ...
    for t in range(int(config.max_timesteps)):
        ...
        batch_seq = replay_buffer_modular_seq.sample(config.batch_size,
                                                     normalize=False)  # normalize only for the sampled batch
